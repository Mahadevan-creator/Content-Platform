...
# # Interview status is updated by the separate interview_poller service (every 30 min).
# # Frontend reads status from /api/experts (MongoDB); no per-request check-status calls.
...
from services.github_service import analyze_repository_contributors, get_github_token
from services.mongodb_service import get_expert, get_expert_by_email, get_mongodb_collection, update_expert_interview, update_expert_interview_completion, update_expert_email_sent, update_expert_contract_sent, get_expert_by_envelope_id, update_expert_contract_status_by_envelope_id, update_expert_from_form, update_expert_assessment_completion
from services.interview_poller import determine_interview_result
import logging

# Test (assessment) result helpers - same pattern as interview webhook in main.py
_TEST_PASS_SCORE_THRESHOLD = 75


def _extract_test_score(data: dict) -> Optional[float]:
    """Extract score (0-100) from HackerRank webhook payload."""
    if not data:
        return None
...
def _extract_test_plagiarism(data: dict) -> bool:
    """Extract plagiarism flag from HackerRank webhook payload."""
    if not data:
        return False

    def _is_plag(val) -> bool:
        if val is None:
            return False
    ...
    for key in ('plagiarism', 'plagiarism_detected', 'plagiarism_flag', 'plagiarism_status',
                'suspicious_activity', 'has_plagiarism', 'is_plagiarism'):
        val = data.get(key)
        if val is not None:
            return _is_plag(val)
...
def _is_test_completed(data: dict) -> bool:
    """Check if test is completed (submitted)."""
    if not data:
        return False
    ...
    if status is not None:
        if isinstance(status, str):
            sl = str(status).lower()
            if sl in ('completed', 'submitted', 'done', 'finished', 'failed'):
                return True
        elif isinstance(status, (int, float)) and int(status) == 7:
            return True
    return _extract_test_score(data) is not None


def _is_test_explicitly_failed(data: dict) -> bool:
    """Check if HackerRank explicitly marks candidate as failed."""
    if not data:
        return False
    status = data.get('status') or data.get('state')
    if status is not None and isinstance(status, str) and str(status).lower() == 'failed':
        return True
    for nested in ('model', 'result', 'data'):
        obj = data.get(nested)
        if isinstance(obj, dict):
...
def _determine_assessment_result(data: dict) -> Optional[tuple[str, float]]:
    """
    Determine assessment (test) result from HackerRank webhook payload.
    Returns (assessment_result, score) or None if not yet completed.
    """
    if not data or not _is_test_completed(data):
        return None
    score = _extract_test_score(data)
    plagiarism = _extract_test_plagiarism(data)
    if _is_test_explicitly_failed(data):
        return ('failed', score if score is not None else 0.0)
    if score is None:
        return ('failed', 0.0)
    passed = score >= _TEST_PASS_SCORE_THRESHOLD and not plagiarism
...
@app.on_event("startup")
async def startup_event():
    # Debug: verify TEST_RESULT_CALLBACK_URL is loaded (critical for test webhook)
    test_cb = os.getenv("TEST_RESULT_CALLBACK_URL", "")
    if test_cb:
        print(f"   TEST_RESULT_CALLBACK_URL={test_cb}")
    else:
        print(f"   TEST_RESULT_CALLBACK_URL=NOT SET (test webhook will NOT work! Add to .env)")
    token = get_github_token()
    if not token:
        print("\n" + "="*80)
        print("⚠️  WARNING: GITHUB_TOKEN environment variable is not set!")
...
# Request logging for test webhook - see when HackerRank calls us
@app.middleware("http")
async def log_test_webhook_requests(request: Request, call_next):
    if "/api/tests/result-callback" in request.url.path and request.method == "POST":
        print(f"\n🔔 INCOMING REQUEST: POST {request.url.path} (HackerRank webhook?)")
...
async def create_interview(request: CreateInterviewRequest):
    ...
    payload = request.model_dump(by_alias=True, exclude_none=True)
    # Inject result_url so HackerRank calls our callback when interview is completed (no polling)
    callback_url = os.getenv("INTERVIEW_RESULT_CALLBACK_URL", "").strip()
    if callback_url and not payload.get("result_url"):
        payload["result_url"] = callback_url
        print(f"✅ Injected result_url for interview callback: {callback_url}")
    url = f"{base_url}/x/api/v3/interviews"
    headers = {
        "Authorization": f"Bearer {api_key}",
        "Content-Type": "application/json",
...
class SendTestRequest(BaseModel):
    """Maps to HackerRank TestCandidate: POST /x/api/v3/tests/{test_id}/candidates"""
    test_id: str  # HackerRank test ID
    candidate_email: str  # required
    candidate_name: Optional[str] = None  # -> full_name
    send_email: Optional[bool] = True
    subject: Optional[str] = None
    message: Optional[str] = None
    test_result_url: Optional[str] = None
    test_finish_url: Optional[str] = None  # Redirect URL when test ends
    template: Optional[str] = None
    evaluator_email: Optional[str] = None
    ats_state: Optional[int] = None  # 0-22
    invite_metadata: Optional[Dict] = None  # JSON object
        ...
        if request.interview_id and not interview_status:
            ...
            if api_key:
                try:
                    ...
                    # Determine result using full logic from interview_poller (thumbs_up, result, feedback regex)
                    if not request.interview_result and interview_status in ('completed', 'ended'):
                        request.interview_result = determine_interview_result(interview_data)
                except Exception as e:
                    print(f"⚠️  Could not fetch interview status from HackerRank: {e}")
...
async def send_test_to_candidate(request: SendTestRequest):
    ...
    # Build payload per HackerRank TestCandidate API spec
    payload: Dict[str, Any] = {
        "email": request.candidate_email.strip(),
        "send_email": request.send_email if request.send_email is not None else True,
    }
    if request.candidate_name:
        payload["full_name"] = request.candidate_name.strip()
    if request.subject:
        payload["subject"] = request.subject
    if request.message:
        payload["message"] = request.message
    if request.test_finish_url:
        payload["test_finish_url"] = request.test_finish_url.strip()
    if request.template:
        payload["template"] = request.template
    if request.evaluator_email:
        payload["evaluator_email"] = request.evaluator_email.strip()
    if request.ats_state is not None and 0 <= request.ats_state <= 22:
        payload["ats_state"] = request.ats_state
    if request.invite_metadata and isinstance(request.invite_metadata, dict):
        payload["invite_metadata"] = request.invite_metadata
    if request.tags and isinstance(request.tags, list):
        payload["tags"] = request.tags

    # COMPULSORY: test_result_url from .env - always send webhook URL when inviting candidate
    callback_url = (os.getenv("TEST_RESULT_CALLBACK_URL", "") or "").strip()
    if not callback_url:
        # Fallback: derive from INTERVIEW_RESULT_CALLBACK_URL (same tunnel, different path)
        interview_cb = (os.getenv("INTERVIEW_RESULT_CALLBACK_URL", "") or "").strip()
        if interview_cb and "/api/interviews/result-callback" in interview_cb:
            callback_url = interview_cb.replace("/api/interviews/result-callback", "/api/tests/result-callback")
    if not callback_url:
        raise HTTPException(
            status_code=503,
            detail="TEST_RESULT_CALLBACK_URL not set in .env. Add TEST_RESULT_CALLBACK_URL (your webhook URL) to .env to send tests.",
        )
    # Always include webhook config in every test invite
    payload["test_result_url"] = callback_url
    payload["accept_result_updates"] = True
    webhook_secret = os.getenv("TEST_WEBHOOK_SECRET", "").strip() or api_key
    payload["webhook_authentication"] = {
        "type": "bearer_token",
    ...
    print(f"📧 Sending test invite: test_id={request.test_id}, email={request.candidate_email}")
    print(f"   URL: {url}")
    print(f"   test_result_url={payload.get('test_result_url') or 'NOT SET'}")
    print(f"   accept_result_updates={payload.get('accept_result_updates')}")
    print(f"   webhook_authentication={'SET' if payload.get('webhook_authentication') else 'NOT SET'}")
    print(f"   FULL PAYLOAD to HackerRank: {json.dumps(payload, default=str)}")
    ...
    try:
        ...
        if collection is not None:
            ...
            if expert:
                ...
                # Build test report URL for viewing candidate's results in HackerRank Work
                test_candidate_id = test_response.get('id')
                if test_candidate_id is not None:
                    test_candidate_id = str(test_candidate_id)
                test_report_url = None
                if request.test_id and test_candidate_id:
                    base = (os.getenv("HACKERRANK_API_BASE") or "https://www.hackerrank.com").rstrip("/")
                    test_report_url = f"{base}/work/tests/{request.test_id}/candidates/{test_candidate_id}"
...
@app.get("/api/tests/result-callback")
@app.head("/api/tests/result-callback")
async def test_result_callback_get():
    """GET/HEAD: HackerRank may validate URL with HEAD/GET before POSTing. Return 200 OK."""
    return {"message": "Test result callback is active. Use POST when test is completed."}


@app.post("/api/tests/result-callback/manual-test")
async def test_result_callback_manual_test(request: Request):
    """Manually simulate HackerRank webhook for debugging. POST a test result payload to verify the flow."""
    try:
        body = await request.json()
        print(f"🔔 MANUAL TEST: Simulating webhook with payload keys={list(body.keys())}")
        test_id = body.get("test_id") or body.get("testId")
        test_candidate_id = body.get("id") or body.get("candidate_id") or body.get("test_candidate_id")
        candidate_email = None
        if body.get("candidate") and isinstance(body["candidate"], dict):
            candidate_email = (body["candidate"].get("email") or "").strip()
        if not candidate_email:
            candidate_email = (body.get("email") or "").strip()
        if not candidate_email and test_id and test_candidate_id:
            collection = get_mongodb_collection()
            if collection:
                expert = collection.find_one({"test_id": str(test_id), "test_candidate_id": str(test_candidate_id)})
                if expert:
                    candidate_email = expert.get("email")
        if not candidate_email:
            return {"success": False, "message": "Need email or (test_id + test_candidate_id) to find candidate"}
        data = _test_result_callback_to_data(body)
        result = _determine_assessment_result(data)
        if result is None:
            return {"success": True, "message": "Test not yet completed (mock)", "data_sample": dict(list(data.items())[:5])}
        assessment_result, score = result
        update_result = update_expert_assessment_completion(
            email=candidate_email,
            assessment_result=assessment_result,
            hrw_score=score,
            test_id=str(test_id) if test_id else None,
            test_candidate_id=str(test_candidate_id) if test_candidate_id else None,
        )
        if update_result:
            print(f"   ✅ Manual test: updated {candidate_email} result={assessment_result} score={score}")
            return {"success": True, "message": f"Updated {candidate_email}", "result": assessment_result, "score": score}
...
def _test_result_callback_to_data(payload: dict) -> dict:
    """Convert test_result_url callback payload to the shape expected by _determine_assessment_result.
    HackerRank may send score/plagiarism/status under different keys or nested in model/result/data.
    """
    # Flatten nested structures - determine_assessment_result checks top-level and nested
    data = dict(payload)
    for nested in ('model', 'result', 'data'):
...
@app.post("/api/tests/result-callback")
async def test_result_callback(request: Request):
    """Callback endpoint for HackerRank test_result_url.
    
    When you send a test with test_result_url pointing to this endpoint (e.g.
    TEST_RESULT_CALLBACK_URL=https://your-api.com/api/tests/result-callback),
    HackerRank POSTs the test result here when the candidate completes the test.
    Uses pass/fail logic: score >= 75 AND no plagiarism.
    """
    try:
        body = await request.json()
        if not isinstance(body, dict):
        ...
        # Debug: ALWAYS print so we see in console when HackerRank calls
        print(f"🔔 TEST RESULT WEBHOOK CALLED! payload keys={list(body.keys())}")

        # HackerRank sends "test" (not test_id), "id" for candidate id
        test_id = body.get("test_id") or body.get("testId") or body.get("test")
        if test_id is not None and not isinstance(test_id, str):
            test_id = str(test_id)
        test_candidate_id = body.get("id") or body.get("candidate_id") or body.get("test_candidate_id") or body.get("candidateId")
        if test_candidate_id is not None and not isinstance(test_candidate_id, str):
            test_candidate_id = str(test_candidate_id)
        ...
        if not candidate_email and (test_id and test_candidate_id):
            collection = get_mongodb_collection()
            if collection:
                # Try match - MongoDB may have test_candidate_id as string or int
                expert = collection.find_one({"test_id": str(test_id), "test_candidate_id": str(test_candidate_id)})
                if not expert:
                    expert = collection.find_one({"test_candidate_id": str(test_candidate_id)})
                if not expert and test_candidate_id:
                    try:
                        expert = collection.find_one({"test_candidate_id": int(test_candidate_id)})
        ...
        if not candidate_email:
            print(f"   ⚠️ Test result-callback: no candidate email, test_id={test_id}, test_candidate_id={test_candidate_id}")
            return {
                "success": False,
                "message": "Could not determine candidate (missing email and no expert found by test_id/test_candidate_id)",
            }

        data = _test_result_callback_to_data(body)
        result = _determine_assessment_result(data)
        if result is None:
            print(f"   Test result-callback: test not yet completed (data sample={dict(list(data.items())[:5])})")
            return {
                "success": True,
                "message": "Test not yet completed, ignoring",
            }
        ...
        if update_result:
            print(f"   ✅ Test result-callback: updated {candidate_email} result={assessment_result} score={score}")
            return {
                "success": True,
                "message": f"Test result updated for {candidate_email}",
                "result": assessment_result,
                "score": score,
            }
        return {
            "success": False,
            "message": f"Failed to update test result for {candidate_email}",
        }
    except HTTPException:
        raise
    except Exception as e:
        logging.exception("Test result-callback error: %s", e)
        raise HTTPException(status_code=500, detail=str(e))
...
# Payload HackerRank POSTs to result_url when interview is completed (response-sample format)
class HackerRankResultCallbackCandidate(BaseModel):
    name: Optional[str] = None
    email: str


class HackerRankResultCallbackPayload(BaseModel):
    """Payload sent by HackerRank to result_url when interview status updates (full interview object)."""
    id: Optional[str] = None  # interview id (may be numeric string)
    status: Optional[str] = None  # e.g. 'new', 'ended', 'completed'
    feedback: Optional[str] = None
    thumbs_up: Optional[int] = None  # 1 = pass, 0 = fail
    candidate: Optional[HackerRankResultCallbackCandidate] = None
    ended_at: Optional[str] = None
    report_url: Optional[str] = None

    class Config:
        extra = "allow"  # Accept any extra fields HackerRank sends


def _result_callback_to_interview_data(payload: dict) -> dict:
    """Convert result_url callback payload to the shape expected by determine_interview_result.
    HackerRank may send feedback under different keys: feedback, interviewer_feedback,
    feedback_text, or nested in interviewer/feedback objects.
    """
    feedback = (
        payload.get("feedback")
        or payload.get("interviewer_feedback")
        or payload.get("feedback_text")
        or (payload.get("interviewer") or {}).get("feedback")
        or (payload.get("interviewer") or {}).get("feedback_text")
    )
    return {
        ...
        "interviewer_recommendation": payload.get("interviewer_recommendation"),
        "feedback_questions": payload.get("feedback_questions"),
    }


@app.post("/api/interviews/result-callback")
@app.post("/api/interviews/result-callback/")
async def interview_result_callback(request: Request):
    """Callback endpoint for HackerRank result_url.

    When you create an interview with result_url pointing to this endpoint (e.g.
    INTERVIEW_RESULT_CALLBACK_URL=https://your-api.com/api/interviews/result-callback),
    HackerRank POSTs the interview object here when the interview is completed or
    status changes. We update the candidate's interview status and result in MongoDB.
    No polling required.
    """
    try:
        body = await request.json()
        ...
        if interview_id is not None and not isinstance(interview_id, str):
            interview_id = str(interview_id)
        status = body.get("status") or ""
        # Normalize: HackerRank may send 'ended' or 'completed'
        interview_status = "ended" if status.lower() in ("ended", "completed") else status

        candidate_email = None
        if body.get("candidate") and isinstance(body["candidate"], dict):
            candidate_email = (body["candidate"].get("email") or "").strip()
        if not candidate_email and interview_id:
            collection = get_mongodb_collection()
            if collection:
                expert = collection.find_one({"interview_id": interview_id})
                if not expert and interview_id is not None:
                    expert = collection.find_one({"interview_id": str(interview_id)})
                if expert:
                    candidate_email = expert.get("email")

        if not candidate_email:
            logging.warning(f"Interview result-callback: no candidate email and could not resolve by interview_id={interview_id}")
            return {
                "success": False,
                "message": "Could not determine candidate (missing candidate.email and no expert found by interview_id)",
            }

        # Only update MongoDB when interview is completed/ended
        if interview_status not in ("ended", "completed"):
            return {
                "success": True,
                "message": f"Ignored non-terminal status: {status}",
                "status": status,
            }

        interview_data_normalized = _result_callback_to_interview_data(body)
        interview_result = determine_interview_result(interview_data_normalized)
        logging.info(f"Interview result-callback: payload keys={list(body.keys())}, thumbs_up={body.get('thumbs_up')}, result={body.get('result')}, feedback_len={len(str(body.get('feedback') or ''))}, parsed_result={interview_result}")
        # If result still blank, fetch full interview from HackerRank API (callback payload may be incomplete)
        if interview_result is None and interview_id:
            interview_data = _fetch_interview_from_hackerrank(interview_id)
            if interview_data:
                interview_result = determine_interview_result(interview_data)
                logging.info(f"Interview result-callback: fetched from API for {interview_id}, api_keys={list(interview_data.keys())}, result={interview_result}")
            else:
                logging.warning(f"Interview result-callback: could not fetch from API for {interview_id}")
        ...
        if update_result:
            logging.info(f"Interview result-callback: updated {candidate_email} status={interview_status} result={interview_result}")
            return {
                "success": True,
                "message": f"Interview status updated for {candidate_email}",
                "status": interview_status,
        ...
        logging.exception("Interview result-callback error: %s", e)
        raise HTTPException(status_code=500, detail=str(e))


def _fetch_interview_from_hackerrank(interview_id: str) -> Optional[dict]:
    """Fetch full interview data from HackerRank API (same logic as interview_poller)."""
    api_key = os.getenv("HACKERRANK_API_KEY")
    base_url = (os.getenv("HACKERRANK_API_BASE") or "https://www.hackerrank.com").rstrip("/")
    if not api_key:
        return None
        ...
        if interview_status in ('completed', 'ended'):
            if payload.thumbs_up is True:
                ...
                if r in ('yes', 'y'):
                    ...
                    interview_result = 'fail'
            # If still blank, fetch full interview from HackerRank and use determine_interview_result
            if interview_result is None and interview_id:
                interview_data = _fetch_interview_from_hackerrank(interview_id)
                if interview_data:
                    interview_result = determine_interview_result(interview_data)
                    logging.info(f"Interview webhook: fetched from API for {interview_id}, result={interview_result}")
...
async def check_all_pending_interviews():
    ...
    This endpoint is designed to be called by a background job/cron.
    Finds experts with workflow.interview = 'scheduled' and interview_id set,
    then checks their status from HackerRank and updates MongoDB.
    
    Result updates rely on HackerRank's 2nd callback when interviewer submits scorecard.
    """
    try:
        collection = get_mongodb_collection()
        if collection is None:
